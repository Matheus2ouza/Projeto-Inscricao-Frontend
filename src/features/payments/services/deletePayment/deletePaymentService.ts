...
export async function deletePaymentService(paymentId: string) {
  try {
    const { data } = await axiosServer.delete(`/payments/${paymentId}`);
    return data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      ...
      if (error.request) {
        ...
        throw new Error(
          'Não foi possível excluir o pagamento no momento. Tente novamente mais tarde.',
        );
      }
    ...
    throw new Error(
      'Ocorreu um erro inesperado ao excluir o pagamento. Tente novamente mais tarde.',
    );
  }
}
