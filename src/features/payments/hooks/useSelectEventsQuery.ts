...
import { getEvents } from "@/features/payments/api/getEvents";
import { useQuery, useQueryClient } from "@tanstack/react-query";
// Chaves de query para organização
export const eventsKeys = {
  all: ["events-payments"] as const,
...
export function useSelectEventsQuery(page: number = 1, pageSize: number = 8, status?: StatusEvent[]) {
  return useQuery({
